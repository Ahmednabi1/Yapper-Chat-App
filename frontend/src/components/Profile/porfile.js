    ...
    const updateProfile = async () => {
        ...
        try {
            const response = await fetch(`/api/profile/${userId}`, {  
                ...
                body: JSON.stringify({ username, email, phone, password, profilePicture: image })
            });
            ...
            <div className="Profile-form">
                <form className="row" onSubmit={(e) => { e.preventDefault(); updateProfile(); }}>
                    ...
                    <div className="input-field">
                        <div className="form-group">
                            <label htmlFor="account-pass">New Password</label>
                            <input className="form-control" type="password" id="account-pass" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Write A Complex Password from 8 Letters" />
                        </div>
                    </div>
                    <div className="input-field">
                        <div className="form-group">
                            <label htmlFor="account-confirm-pass">Confirm Password</label>
                            <input className="form-control" type="password" id="account-confirm-pass" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Repeat the password" />
                        </div>
                    </div>
                    {errorMessage && <div className="alert alert-danger">{errorMessage}</div>}
                    {successMessage && <div className="alert alert-success">{successMessage}</div>}
